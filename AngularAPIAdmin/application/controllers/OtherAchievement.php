...
class OtherAchievement extends REST_Controller {	
	public function __construct() {
		parent::__construct();
		$this->load->helper('check_token');		
		$this->load->model('OtherAchievement_modal');		
	...
	public function index_post(){
		...
		$Insert_Array = array(
			"`user_id`" => $this->post('user_id'),
			"`achievementarray`" => $this->post('insertArray')
		);
		...
		if(true){
			...
			if($token_status == TRUE){
				$Check_Availability_Result = $this->OtherAchievement_modal->Check_Skills_Availability($user_id);
				if($Check_Availability_Result){
					$Insert_saveUser_Result = $this->OtherAchievement_modal->Update_Skills($Insert_Array,$user_id);
					...
					$Insert_saveUser_Result = $this->OtherAchievement_modal->Insert_Skills($Insert_Array,$user_id);
	...
	public function getOtherAchievement_post(){
		$valid = ['status' => "true","statuscode" => 200,'response' =>"Token Valid"];
		$no_found = ['status' => "true","statuscode" => 200,'response' =>"No Record Found"];
		$invalid = ['status' => "true","statuscode" => 203,'response' =>"In-Valid token"];
		$not_found = ['status' => "true","statuscode" => 404,'response' =>"Token not found"];
		...
		$token_status = check_token($user_id,$headers['Authorization']);
		if($token_status == TRUE){
			$Check_Availability_Result = $this->OtherAchievement_modal->workSkillsData($user_id);
			if($Check_Availability_Result){
