...
class Mdl_common extends CI_Model
{
		...
		if($this->session->userdata('eb-user') == "" ){
			...
			if($accountArr['user_service_type'] == '0'){
	           ...
	           if($accountArr['user_setup'] == 'STEP1' && $accountArr['user_service_type'] == '0'){
                    ...
                    if($this->uri->segment(2) != 'workverification' && $this->uri->segment(2) != 'profile'){
                       // redirect(''.$usertype.'/workverification');
                    }
                    ...
                    if($this->uri->segment(2) != 'paymentinformation' && $this->uri->segment(2) != 'workverification' && $this->uri->segment(2) != 'profile' && $this->uri->segment(3) != 'upload'){
                        redirect(''.$usertype.'/paymentinformation');
                    }
	        	}else if($accountArr['user_setup'] == 'STEP4' && $accountArr['user_service_type'] == '0'){
	        		if($this->uri->segment(2) != 'securityquestion' && $this->uri->segment(2) != 'paymentinformation' && $this->uri->segment(2) != 'workverification' && $this->uri->segment(2) != 'profile' && $this->uri->segment(3) != 'upload'){
	...
	function uploadFile($uploadFile,$filetype,$folder,$fileName='')
	{
		...
		if($filetype == 'img') 	$config['allowed_types'] = 'gif|jpg|png|jpeg|csv|mp4|3gp|vob|flv|mp3';
	...
	function sendMail($to,$from,$subject,$message,$attachmentName='',$attachmentFiled='')
	{
        if($_SERVER['HTTP_HOST'] == 'localhost'){
			...
			$m->sendThroughSMTP("smtp.gmail.com", 465, 'dv.sparkle016@gmail.com', 'Sparkle2#', true);/*akash*/ 
			$m->send();  
        ...
        function getCalenderDetails($userType,$userId,$date=''){
            ...
            if($userType == 'provider'){
                $getJobQuery="select *,eb_services_job.user_id as sj_user_id from eb_services_job where del_in=0  and sj_id in (select sj_id from eb_services_job_application where user_id='".$userId."' and (status='ONGOING_IN_PROGRESS')) and '".$ndate."' between date(STR_TO_DATE(schedule_start_date,'%m/%d/%Y')) and date(STR_TO_DATE(schedule_end_date,'%m/%d/%Y')) ";
                
            }
            else{
                //and '".$date."' between schedule_start_date and schedule_end_date;
                $getJobQuery="select *,eb_services_job.user_id as sj_user_id from eb_services_job where sj_id in(select sj_id from eb_services_job_application where (status='ONGOING_IN_PROGRESS')) and user_id=$userId and del_in=0 and '".$ndate."' between STR_TO_DATE(schedule_start_date,'%m/%d/%Y') and STR_TO_DATE(schedule_end_date,'%m/%d/%Y') ";
    ...
    function sendIosNotification($message, $deviceTokens, $pushType = array(), $data = array(), $userInfo = array()) {
        ...
        if (!$fp)
           // exit("Failed to connected: $err $errstr" . PHP_EOL);
              exit();
